...
def repeat_classification(ref_adata, label_key='labels', classifier=get_knn_classification_report, rep=5, seed=12345):
    # Set the seed for reproducibility
    np.random.seed(seed)

    # Initialize lists to store the output of each method
    output_random = []
    output_fsbs = []
    output_proportional = []

    # Set the maximum seed
    MAX_SEED = 10000
    ...
    for _ in range(rep):
        ...
        # Apply the classifier to each method and store the output
        ...
        output_proportional.append(classifier(prop_s, ref_adata, label_key=label_key)[0])

    # Return the output of each method
    return output_random, output_fsbs, output_proportional
